...
/**
 * WS Edge simple: envía 4 fixtures rotativos cada 2s.
 * Ruta: wss://<tu-dominio>/api/qcore
 */
const FIXTURES = [
...
export default function handler(req: Request) {
  // Health check opcional: GET /api/qcore?ping=1 -> "ok"
  const { searchParams } = new URL(req.url);
  if (searchParams.get('ping') === '1') return new Response('ok', { status: 200 });

  // Upgrade a WebSocket (Edge: WebSocketPair)
  // @ts-ignore - tipos aún no estándar
  ...
  // Heartbeat cada 10s (útil para saber que sigue vivo)
  const hb = setInterval(() => {
    try { server.send(JSON.stringify({ t:'heartbeat', ts: Date.now() })); } catch {}
  }, 10000);

  // Enviar fixtures rotativos cada 2s
  let i = 0;
  const loop = setInterval(() => {
    try {
      const msg = { ...FIXTURES[i++ % FIXTURES.length], ts: Date.now() };
      server.send(JSON.stringify(msg));
    } catch {
      // si falla el send, el cliente probablemente cerró
    }
  }, 2000);
  ...
  // Responder con 101 Switching Protocols y devolver el "client"
  // @ts-ignore
